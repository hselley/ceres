...
{
	$res = seleccionar("select * from registro where id = ".$cual." and status = 'ACTI'");
...
{
	$res = seleccionar("select * from registro where email = '".$usuario."'");
	if (mysqli_num_rows($res) > 0)
	{
		$row = mysqli_fetch_array($res);
		$r[0] = $row["ID"];
