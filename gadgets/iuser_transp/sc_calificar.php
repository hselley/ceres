...
<?php }
if($yacr['ID_Transportista']!=$_SESSION["NumTransportista"]){ ?>
  <script type="text/javascript">
    window.location="transportista.php"
  </script>
<?php }
...
<section class="main-content">
  <div class="row">
              ...
              <div class="accordion-inner">
                <div class="row-fluid">
                  <?php
                    ...
                    $r = seleccionar("SELECT p.ID idped, p.TArticulos artp, pd.Nombre nprod, pd.Tipo tprod, pd.Medida mprod, pd.Foto, pd.Disponible dprod, pr.Nombre prnom, pr.Apellido prape, pr.Email premail, pr.Telefono pretel, c.Nombre cnom, c.Email cemail, c.Telefono ctel FROM pedido p, historial h, producto pd, productor_2 pr, cliente c WHERE p.ID=".$pedido." AND FechaFin='NO' AND c.ID=p.ID_Cliente AND p.ID=h.ID_Pedido AND h.ID_Producto=pd.ID AND pd.Productor=pr.ID");
                    ...
                    while ($row = mysqli_fetch_array($r)) {
                      echo "<tr>
                          ...
                          <tr>
                            <td><img src=\"producto/".$row3['Foto']."\" height='50%'></td>
                            ...
                            <td>".$row['cnom']."<br>
                                  <strong>Email:</strong> ".$row['cemail']."<br>
                                  <strong>Teléfono:</strong> ".$row['ctel']." <br>
                        </tr>";
                    }
                    echo "</table></div>";
                  ?>
    ...
    <div class="col-sm-12" align="center">
      <button class="btn btn-success" onClick="window.location='transportista.php'">PERFIL DE USUARIO</button>
    </div>
  </div>
</section>
</section>
